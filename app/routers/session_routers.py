...
def session_detail(request: Request, session_id: int, db: Session = Depends(get_db)):
    """
    Отображает страницу с деталями сеанса фильма для пользователя.
    Проверяет токен и существование пользователя в базе.
    Если пользователь уже забронировал этот сеанс, делает редирект на страницу брони.
    Иначе отображает шаблон 'movie_detail_home.html' с информацией о сеансе.
    """
